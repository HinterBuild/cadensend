...
CONNECTORS: List[CatalogItem] = [
    CatalogItem("linear", "connector", "Linear", "Pull completed issues into a team newsletter.", "project", {"auth": "oauth", "scopes": ["read"], "sync": "issues"}),
    CatalogItem("clickup", "connector", "ClickUp", "Summarize tasks by sprint, owner, and status.", "project", {"auth": "oauth", "sync": "tasks"}),
    CatalogItem("github", "connector", "GitHub", "Turn PRs, releases, and discussions into issue drafts.", "dev", {"auth": "oauth", "sync": "prs,releases,discussions"}),
    CatalogItem("jira", "connector", "Jira", "Convert sprint outcomes into internal status emails.", "project", {"auth": "oauth", "sync": "sprints"}),
    CatalogItem("notion", "connector", "Notion", "Ingest page databases as structured editorial inputs.", "docs", {"auth": "oauth", "sync": "databases,pages"}),
    CatalogItem("google_drive", "connector", "Google Drive", "Watch selected folders for new source material.", "storage", {"auth": "oauth", "sync": "folder_watch"}),
    CatalogItem("dropbox", "connector", "Dropbox", "Sync research documents into the source library.", "storage", {"auth": "oauth", "sync": "files"}),
    CatalogItem("airtable", "connector", "Airtable", "Use editorial calendars and source tables as generation inputs.", "data", {"auth": "api_key", "sync": "tables"}),
    CatalogItem("gmail", "connector", "Gmail", "Convert labeled threads into source packs.", "email", {"auth": "oauth", "sync": "labeled_threads"}),
    CatalogItem("slack", "connector", "Slack", "Summarize channel activity into recurring briefings.", "chat", {"auth": "oauth", "sync": "channels"}),
    CatalogItem("discord", "connector", "Discord", "Extract community sentiment and top questions.", "chat", {"auth": "bot_token", "sync": "channels"}),
    CatalogItem("trello", "connector", "Trello", "Turn board movement into weekly progress updates.", "project", {"auth": "oauth", "sync": "boards"}),
    CatalogItem("asana", "connector", "Asana", "Build project newsletters from milestone changes.", "project", {"auth": "oauth", "sync": "projects"}),
    CatalogItem("figma", "connector", "Figma", "Turn design review notes into product-update copy.", "design", {"auth": "oauth", "sync": "comments"}),
    CatalogItem("google_calendar", "connector", "Google Calendar", "Create event-preview or event-recap newsletters.", "calendar", {"auth": "oauth", "sync": "events"}),
    CatalogItem("crm", "connector", "CRM", "Include pipeline or customer trend commentary.", "sales", {"auth": "api_key", "sync": "deals,contacts"}),
    CatalogItem("analytics", "connector", "Analytics", "Turn KPI deltas into narrative updates.", "analytics", {"auth": "api_key", "sync": "metrics"}),
    CatalogItem("rss_aggregator", "connector", "RSS Aggregator", "Bundle multiple feeds into one digest job.", "feeds", {"auth": "none", "sync": "feeds"}),
    CatalogItem("cms", "connector", "CMS", "Publish approved issues as blog posts automatically.", "publish", {"auth": "api_key", "sync": "publish"}),
    CatalogItem("email_provider", "connector", "Email Provider", "Draft, send, and measure in one workflow run.", "publish", {"auth": "api_key", "sync": "send,metrics"}),
]
