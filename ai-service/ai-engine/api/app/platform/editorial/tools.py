...
class EvaluationHarness:
    ...
    def evaluate(self, issue: Dict[str, Any], prior_issues: List[Dict[str, Any]] | None = None) -> Dict[str, Any]:
        ...
        grounding = cited / len(blocks) if blocks else 0

        overlap = 0.0
        if prior_issues and text:
            prior_text = " ".join(
                " ".join(b.get("text", "") for b in p.get("content_blocks", []))
                for p in prior_issues[-2:]
            )
            words_a = set(WORD_RE.findall(text.lower()))
            words_b = set(WORD_RE.findall(prior_text.lower()))
            if words_a:
                overlap = len(words_a & words_b) / len(words_a)

        has_structure = len(blocks) >= 2 and bool(issue.get("subject"))
        ...
        return {
            "passed": overall >= 0.6 and not risky,
