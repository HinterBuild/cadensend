  ...
  return (
    <div className="text-center py-8" role="status" aria-live="polite">
      <div className="mb-4 flex justify-center">
        ...
        {token && email && state === 'ok' && <CheckCircle2 className="h-12 w-12 text-green-600" aria-hidden="true" />}
        {(!token || !email || state === 'error') && <XCircle className="h-12 w-12 text-red-600" aria-hidden="true" />}
      </div>
      <h2 className="text-lg font-semibold text-gray-900 mb-2">
        {token && email && state === 'ok' ? 'Subscription confirmed' : (!token || !email || state === 'error') ? 'Verification failed' : 'One moment…'}
      </h2>
      <p className="text-gray-600" role={!token || !email || state === 'error' ? 'alert' : undefined}>{!token || !email ? 'This verification link is incomplete. Please use the link from your email.' : message}</p>
    </div>
  );
}
